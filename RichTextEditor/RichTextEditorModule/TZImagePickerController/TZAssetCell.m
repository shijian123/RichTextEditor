...
- (void)setType:(TZAssetCellType)type {
    ...
    } else if ((type == TZAssetCellTypePhotoGif && self.allowPickingGif) || (type == TZAssetCellTypePhotoGif && self.showGifTag)) {
        self.bottomView.hidden = NO;
...
- (void)refreshSelectImageViewStatus:(BOOL)status {
    self.selectImageView.image = status ? self.photoSelImage : self.photoDefImage;
}
